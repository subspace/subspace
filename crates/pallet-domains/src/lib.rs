...
use sp_domains_fraud_proof::fraud_proof::{
    ...
    InvalidDomainBlockHashProofV2, InvalidTransfersProofV2,
};
use sp_domains_fraud_proof::storage_proof::{self, BasicStorageProof, DomainRuntimeCodeProof};
use sp_domains_fraud_proof::verification_v2;
...
pub type FraudProofV2For<T> = FraudProofV2<
    BlockNumberFor<T>,
    <T as frame_system::Config>::Hash,
    <T as Config>::DomainHeader,
    <T as Config>::MmrHash,
...
mod pallet {
    ...
    use crate::{
        ...
        FraudProofV2For, HoldIdentifier, NominatorId, OpaqueBundleOf, ReceiptHashFor, StateRootOf,
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn submit_fraud_proof(
            origin: OriginFor<T>,
            fraud_proof: Box<FraudProofV2For<T>>,
        ) -> DispatchResultWithPostInfo {
            ensure_none(origin)?;
...
impl<T: Config> Pallet<T> {
    ...
    fn validate_fraud_proof_v2(
        ...
        match &fraud_proof.proof {
            FraudProofVariant::InvalidBlockFees(InvalidBlockFeesProofV2 { storage_proof }) => {
            ...
            FraudProofVariant::InvalidTransfers(InvalidTransfersProofV2 { storage_proof }) => {
                let domain_runtime_code = Self::get_domain_runtime_code_for_receipt(
                    domain_id,
                    &bad_receipt,
                    fraud_proof.maybe_domain_runtime_code_proof.clone(),
    ...
    T: Config + frame_system::offchain::SendTransactionTypes<Call<T>>,
{
    ...
    pub fn submit_fraud_proof_unsigned(fraud_proof: FraudProofV2For<T>) {
